% =====================================================================
%  Guide — Environment Setup: ECS + SSH Proxy + OpenCode + Huawei Project
...
\setheadertitle{Huawei Cloud -- ECS + SSH + OpenCode Setup}
...
\begin{objetivos}
  ...
  serve as the development environment for OpenCode and the Huawei document
  templates project. By the end of this chapter you will have a running ECS
  instance with a public IP accessible over SSH.}
...
% =====================================================================
%  Chapter 3 — Install OpenCode
% =====================================================================
\section{Install OpenCode on the ECS Instance}

\begin{objetivos}
  \objgeral{Install OpenCode on the remote ECS instance so you can create
  and edit Huawei Cloud guide documents from VS Code's remote session.}

  \prerequisitos
  \begin{itemize}
    \item Chapter 2 completed — VS Code connected to the ECS via Remote-SSH.
    \item Node.js 18+ available on the instance (or permission to install it).
  \end{itemize}
\end{objetivos}
...
\passoapasso
\begin{enumerate}
  \item In the VS Code remote terminal, install Node.js via NodeSource:
...
\subsection{Install OpenCode}

\passoapasso
\begin{enumerate}
  \item Install OpenCode globally via npm:
...
\begin{dica}
If you are behind a proxy, configure npm before installing:
\code{npm config set proxy http://proxy.corp.com:8080} and
\code{npm config set https-proxy http://proxy.corp.com:8080}
\end{dica}
...
\begin{objetivos}
  ...
  opencode skill are ready for use.}

  \prerequisitos
  \begin{itemize}
    \item Chapter 3 completed — OpenCode installed and running.
...
cd Huawei-doc-templates
  ...
  \item Run the setup script (installs XeLaTeX, latexmk, fonts, the opencode
        skill, and the VS Code LaTeX Workshop extension):
...
\passoapasso
\begin{enumerate}
  \item In the VS Code remote terminal, launch OpenCode:
...
/skill huawei-template-guide
  ...
  \item The skill will ask for:
    \begin{itemize}
      ...
      \item \textbf{Filename} — e.g. \code{obs-guide.tex}
      \item \textbf{Location} — where to create the project
    \end{itemize}
...
\code{/skill huawei-template-guide} in OpenCode.}
