...
\setheaderlogo{assets/huawei-logo-header.png}
\setcoverlogo{assets/huawei-logo-cover.png}
...
\begin{infobox}
This document demonstrates the \code{guide} template (\code{guide.cls}) for
...
badge, changelog, and the \code{\textbackslash menu} command for menu paths.
\end{infobox}
...
\codigoarquivo[bash]{assets/example-script.sh}
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    \toprule
    \textbf{Flavor} & \textbf{vCPU / RAM} & \textbf{Use case} \\
    \midrule
...
\imagemplaceholder{assets/ecs-flavors.png}{Screenshot of the ECS flavor selection page in the Console}
